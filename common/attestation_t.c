...
static void ecall_get_enclave_format_settings(
    ...
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    get_enclave_format_settings_args_t* _pargs_in = (get_enclave_format_settings_args_t*)input_buffer;
    get_enclave_format_settings_args_t* _pargs_out = (get_enclave_format_settings_args_t*)output_buffer;
    ...
    if (_pargs_in->format_settings)
        OE_SET_OUT_POINTER(format_settings, 1, sizeof(format_settings_t), format_settings_t*);
    ...
    /* Call user function. */
    _pargs_out->oe_retval = get_enclave_format_settings(
        (const oe_uuid_t*)_pargs_in->format_id,
        _pargs_in->format_settings);

    /* Compute the size for the deep-copy out buffer. */
    if (_pargs_in->format_settings && _pargs_in->format_settings->buffer)
        OE_ADD_ARG_SIZE(_deepcopy_out_buffer_size, 1, _pargs_in->format_settings->size);
    ...
    if (_pargs_in->format_settings && _pargs_in->format_settings->buffer)
        OE_WRITE_DEEPCOPY_OUT_PARAM(_pargs_in->format_settings->buffer, 1, _pargs_in->format_settings->size);
    ...
    *output_bytes_written = _output_buffer_offset;
    ...
    if (_pargs_in->format_settings)
    {
        ...
        {
            free(_pargs_in->format_settings[_i_1].buffer);
        }
    }
...
static void ecall_get_evidence_with_public_key(
    uint8_t* input_buffer,
    size_t input_buffer_size,
    uint8_t* output_buffer,
    size_t output_buffer_size,
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    get_evidence_with_public_key_args_t* _pargs_in = (get_evidence_with_public_key_args_t*)input_buffer;
    get_evidence_with_public_key_args_t* _pargs_out = (get_evidence_with_public_key_args_t*)output_buffer;
    ...
    if (_pargs_in->format_id)
        OE_SET_IN_POINTER(format_id, 1, sizeof(oe_uuid_t), oe_uuid_t*);
    if (_pargs_in->format_settings)
        OE_SET_IN_POINTER(format_settings, 1, sizeof(format_settings_t), format_settings_t*);
    if (_pargs_in->format_settings && _pargs_in->format_settings->buffer)
        OE_SET_IN_POINTER(format_settings->buffer, 1, _pargs_in->format_settings->size, uint8_t*);

    /* Set out and in-out pointers. */
    /* In-out parameters are copied to output buffer. */
    if (_pargs_in->pem_key)
        OE_SET_OUT_POINTER(pem_key, 1, sizeof(pem_key_t), pem_key_t*);
    if (_pargs_in->evidence_buffer)
        OE_SET_OUT_POINTER(evidence_buffer, 1, sizeof(evidence_t), evidence_t*);
    ...
    /* Call user function. */
    _pargs_out->oe_retval = get_evidence_with_public_key(
        (const oe_uuid_t*)_pargs_in->format_id,
        _pargs_in->format_settings,
        _pargs_in->pem_key,
        _pargs_in->evidence_buffer);

    /* Compute the size for the deep-copy out buffer. */
    if (_pargs_in->pem_key && _pargs_in->pem_key->buffer)
        OE_ADD_ARG_SIZE(_deepcopy_out_buffer_size, 1, _pargs_in->pem_key->size);
    if (_pargs_in->evidence_buffer && _pargs_in->evidence_buffer->buffer)
        OE_ADD_ARG_SIZE(_deepcopy_out_buffer_size, 1, _pargs_in->evidence_buffer->size);
    ...
    /* Serialize the deep-copied content into the buffer. */
    if (_pargs_in->pem_key && _pargs_in->pem_key->buffer)
        OE_WRITE_DEEPCOPY_OUT_PARAM(_pargs_in->pem_key->buffer, 1, _pargs_in->pem_key->size);
    if (_pargs_in->evidence_buffer && _pargs_in->evidence_buffer->buffer)
        OE_WRITE_DEEPCOPY_OUT_PARAM(_pargs_in->evidence_buffer->buffer, 1, _pargs_in->evidence_buffer->size);
    ...
    *output_bytes_written = _output_buffer_offset;
    ...
    if (_pargs_in->pem_key)
    {
        for (size_t _i_1 = 0; _i_1 < 1; _i_1++)
        {
            free(_pargs_in->pem_key[_i_1].buffer);
        }
    }
    if (_pargs_in->evidence_buffer)
    {
        for (size_t _i_1 = 0; _i_1 < 1; _i_1++)
        {
            free(_pargs_in->evidence_buffer[_i_1].buffer);
        }
    }
...
static void ecall_verify_evidence_and_set_public_key(
    uint8_t* input_buffer,
    size_t input_buffer_size,
    uint8_t* output_buffer,
    size_t output_buffer_size,
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    verify_evidence_and_set_public_key_args_t* _pargs_in = (verify_evidence_and_set_public_key_args_t*)input_buffer;
    verify_evidence_and_set_public_key_args_t* _pargs_out = (verify_evidence_and_set_public_key_args_t*)output_buffer;
    ...
    if (_pargs_in->format_id)
        OE_SET_IN_POINTER(format_id, 1, sizeof(oe_uuid_t), oe_uuid_t*);
    if (_pargs_in->pem_key)
        OE_SET_IN_POINTER(pem_key, 1, sizeof(pem_key_t), pem_key_t*);
    if (_pargs_in->pem_key && _pargs_in->pem_key->buffer)
        OE_SET_IN_POINTER(pem_key->buffer, 1, _pargs_in->pem_key->size, uint8_t*);
    if (_pargs_in->evidence)
        OE_SET_IN_POINTER(evidence, 1, sizeof(evidence_t), evidence_t*);
    if (_pargs_in->evidence && _pargs_in->evidence->buffer)
        OE_SET_IN_POINTER(evidence->buffer, 1, _pargs_in->evidence->size, uint8_t*);
    ...
    /* Call user function. */
    _pargs_out->oe_retval = verify_evidence_and_set_public_key(
        (const oe_uuid_t*)_pargs_in->format_id,
        _pargs_in->pem_key,
        _pargs_in->evidence);
...
static void ecall_generate_encrypted_message(
    uint8_t* input_buffer,
    size_t input_buffer_size,
    uint8_t* output_buffer,
    size_t output_buffer_size,
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    generate_encrypted_message_args_t* _pargs_in = (generate_encrypted_message_args_t*)input_buffer;
    generate_encrypted_message_args_t* _pargs_out = (generate_encrypted_message_args_t*)output_buffer;
    ...
    if (_pargs_in->message)
        OE_SET_OUT_POINTER(message, 1, sizeof(message_t), message_t*);
    ...
    /* Call user function. */
    _pargs_out->oe_retval = generate_encrypted_message(
        _pargs_in->message);

    /* Compute the size for the deep-copy out buffer. */
    if (_pargs_in->message && _pargs_in->message->data)
        OE_ADD_ARG_SIZE(_deepcopy_out_buffer_size, 1, _pargs_in->message->size);
    ...
    /* Serialize the deep-copied content into the buffer. */
    if (_pargs_in->message && _pargs_in->message->data)
        OE_WRITE_DEEPCOPY_OUT_PARAM(_pargs_in->message->data, 1, _pargs_in->message->size);
    ...
    *output_bytes_written = _output_buffer_offset;
    ...
    if (_pargs_in->message)
    {
        for (size_t _i_1 = 0; _i_1 < 1; _i_1++)
        {
            free(_pargs_in->message[_i_1].data);
        }
    }
...
static void ecall_process_encrypted_message(
    uint8_t* input_buffer,
    size_t input_buffer_size,
    uint8_t* output_buffer,
    size_t output_buffer_size,
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    process_encrypted_message_args_t* _pargs_in = (process_encrypted_message_args_t*)input_buffer;
    process_encrypted_message_args_t* _pargs_out = (process_encrypted_message_args_t*)output_buffer;
    ...
    /* Set in and in-out pointers. */
    if (_pargs_in->message)
        OE_SET_IN_POINTER(message, 1, sizeof(message_t), message_t*);
    if (_pargs_in->message && _pargs_in->message->data)
        OE_SET_IN_POINTER(message->data, 1, _pargs_in->message->size, uint8_t*);
    ...
    /* Call user function. */
    _pargs_out->oe_retval = process_encrypted_message(
        _pargs_in->message);
...
oe_ecall_func_t oe_ecalls_table[] = {
    (oe_ecall_func_t) ecall_get_enclave_format_settings,
    (oe_ecall_func_t) ecall_get_evidence_with_public_key,
    (oe_ecall_func_t) ecall_verify_evidence_and_set_public_key,
    (oe_ecall_func_t) ecall_generate_encrypted_message,
    (oe_ecall_func_t) ecall_process_encrypted_message,
